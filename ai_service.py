...
2. explicit_notes: Konkretne, fizyczne nuty zapachowe i składniki (np. "wanilia", "róża", "piżmo", "drzewo sandałowe").
...
4. evidence: Dosłowny cytat z tekstu użytkownika, będący bezpośrednim dowodem na wartość wpisaną w polu abstract_mood.
   - ZASADA AUDYTU: Wartość musi być w 100% dokładnym wycinkiem (kopiuj-wklej) z oryginalnego zapytania.
...
- REGUŁA BRAKU DANYCH (NULL): Jeśli zapytanie ma charakter czysto techniczny i nie zawiera określeń nastroju (np. "Tom Ford z piżmem"), pola `abstract_mood` oraz `evidence` MUSZĄ bezwzględnie przyjąć wartość `null`. Złamanie tej zasady (np. użycie pustego stringa "") spowoduje awarię systemu. Jeśli jedno z tych pól to `null`, drugie też musi być `null`.

### PRZYKŁADY WZORCOWE (FEW-SHOT)

Input: "Szukam czegoś eleganckiego od Diora na randkę, z mocną nutą wanilii"
Output: {"brands": ["Dior"], "explicit_notes": ["wanilia"], "abstract_mood": "eleganckie na randkę", "evidence": "czegoś eleganckiego od Diora na randkę"}

Input: "Pokaż perfumy marki Tom Ford zawierające piżmo"
Output: {"brands": ["Tom Ford"], "explicit_notes": ["piżmo"], "abstract_mood": null, "evidence": null}

Input: "szanel o zapachu cytrusów na upalne lato"
Output: {"brands": ["Chanel"], "explicit_notes": ["cytrusy"], "abstract_mood": "na upalne lato", "evidence": "na upalne lato"}
"""
